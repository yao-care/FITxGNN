...
由於 Fimea 基本註冊 XML 無法穩定批量下載（伺服器回傳 HTML），
改用 EMA Medicines Report (集中授權藥品) 作為資料來源。
EMA 集中授權藥品在所有 EU/EEA 成員國（含芬蘭）有效。

使用方法:
    uv run python scripts/process_fda_data.py

資料來源:
    主要: EMA Medicines Report (集中授權藥品，含芬蘭)
    ...
    參考: Fimea 基本註冊 (XML, 目前無法穩定下載)
          https://data.pilvi.fimea.fi/avoin-data/Perusrekisteri.xml

產生檔案:
    data/raw/fi_fda_drugs.json
"""
...
def download_ema_data(output_path: Path) -> Path:
    ...
    raise FileNotFoundError(
        ...
        f"方法 C: Fimea 基本註冊 (若可用)\n"
        f"  1. 前往 https://fimea.fi/en/databases_and_registers/basic-register-xml\n"
        f"  2. 下載 Perusrekisteri.xml\n"
        f"  3. 將檔案放至 {output_path.parent}/\n\n"
...
def find_data_file(raw_dir: Path) -> Path | None:
    ...
    candidates = [
        ...
        raw_dir / "fimea_basic_register.csv",
        raw_dir / "fimea_basic_register.json",
    ]

    for candidate in candidates:
        if candidate.exists():
            # 驗證不是 HTML（之前下載失敗的 XML 實際是 HTML）
            with open(candidate, "rb") as f:
                head = f.read(50)
            if b"<!doctype" in head.lower() or b"<html" in head.lower():
                print(f"  跳過 {candidate.name}（內容為 HTML，非有效資料）")
                continue
            return candidate

    # 搜尋任何資料檔案（排除已知無效的 XML）
    for pattern in ["ema*.xlsx", "*.csv", "*.json", "*.xlsx", "*.xls"]:
        files = list(raw_dir.glob(pattern))
        for f in files:
            with open(f, "rb") as fh:
                head = fh.read(50)
            if b"<!doctype" not in head.lower() and b"<html" not in head.lower():
                return f
...
def _filter_ema_for_finland(df: pd.DataFrame) -> pd.DataFrame:
    """從 EMA 資料中篩選芬蘭適用的記錄

    EMA Medicines Report 為集中授權，在所有 EU 成員國有效（含芬蘭）。
    ...
    if country_cols:
        ...
        fi_mask = df[country_col].str.contains(
            r"Finland|Suomi|FI\b",
            case=False, na=False, regex=True,
        )
        fi_count = fi_mask.sum()
        print(f"芬蘭相關記錄: {fi_count:,} / {len(df):,}")

        if fi_count > 0:
            df = df[fi_mask].copy()
            print(f"已篩選為芬蘭記錄: {len(df):,} 筆")
        else:
            print("未找到芬蘭特定記錄，保留所有記錄")
...
def main():
    print("=" * 60)
    print("處理芬蘭 Fimea / EMA 藥品資料")
    print("=" * 60)
    print()
    print("注意: Fimea 基本註冊 XML 無法穩定批量下載。")
    print("使用 EMA 資料作為替代來源 (集中授權藥品在芬蘭有效)。")
    print()

    base_dir = Path(__file__).parent.parent
    raw_dir = base_dir / "data" / "raw"
    output_path = raw_dir / "fi_fda_drugs.json"

    # 確保 raw 目錄存在
    raw_dir.mkdir(parents=True, exist_ok=True)

    # 清除已知無效的 HTML 檔案
    invalid_xml = raw_dir / "fimea_basic_register.xml"
    if invalid_xml.exists():
        with open(invalid_xml, "rb") as f:
            head = f.read(50)
        if b"<!doctype" in head.lower() or b"<html" in head.lower():
            print(f"移除無效的 HTML 檔案: {invalid_xml.name}")
            invalid_xml.unlink()
